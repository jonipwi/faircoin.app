                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-gray-600 dark:text-gray-400">
                          {t('governance.totalParticipation')}: <span className="font-bold text-gray-900 dark:text-white">{totalVotes.toLocaleString()} {t('governance.votes')} ({proposal.current_participation.toFixed(1)}%)</span>
                        </span>
                        <span className="text-gray-600 dark:text-gray-400">
                          {t('governance.quorum')}: <span className="font-bold text-gray-900 dark:text-white">{proposal.quorum_percentage}%</span>
                        </span>
                      </div>
                      ...
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-gray-600 dark:text-gray-400">
                            {t('governance.for')}: <span className="font-bold text-gray-900 dark:text-white">{proposal.votes_for.toLocaleString()} ({forPercentage.toFixed(1)}%)</span>
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="w-3 h-3 rounded-full bg-red-500" />
                          <span className="text-sm text-gray-600 dark:text-gray-400">
                            {t('governance.against')}: <span className="font-bold text-gray-900 dark:text-white">{proposal.votes_against.toLocaleString()} ({againstPercentage.toFixed(1)}%)</span>
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              {recentVotes.map((vote, index) => (
                <div key={index} className="card p-4">
                  ...
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">
                    {vote.vote === 'for' ? t('governance.votedFor') : t('governance.votedAgainst')} {vote.proposal_id}
                  </p>
                  {vote.reason && (
                    ...
                    <div key={range} className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        <div className="text-sm font-medium text-gray-900 dark:text-white">
                          {data.count} {t('governance.users')}
                        </div>
                        <div className="text-xs text-gray-500">
                          {data.total_power.toFixed(1)} {t('governance.power')}
                        </div>
                      </div>
                    </div>
                  ))}
